  ...
  useEffect(() => {
    ...
    const fetchTxs = async () => {
      try {
        ...
        if (isMounted) {
          const validTxs = Array.isArray(data) ? data.filter(tx => tx && tx.id && Array.isArray(tx.inputs) && Array.isArray(tx.outputs)) : [];
        ...
        if (isMounted) setTxs([]); // Clear transactions on error
      } finally {
    ...
    fetchTxs();
    const interval = setInterval(fetchTxs, 6000);
    return () => {
      isMounted = false;
